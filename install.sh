...
# JustAGuy Linux - DK Window Manager Setup
# https://github.com/drewgrif/dk-setup
...
CONFIG_DIR="$HOME/.config/dk"
TEMP_DIR="/tmp/dk_$$"
LOG_FILE="$HOME/dk-install.log"
...
export_packages() {
    echo "=== DK Window Manager Setup - Package Lists for Different Distributions ==="
    echo
    ...
    echo "  $0 --only-config    # To copy just the DK configuration files"
}
...
read -p "Install DK Window Manager? (y/n) " -n 1 -r
echo
[[ ! $REPLY =~ ^[Yy]$ ]] && exit 1
...
PACKAGES_CORE=(
    ...
    libxcb-randr0-dev libxcb-util-dev libxcb-icccm4-dev
    libxcb-cursor-dev libxcb-keysyms1-dev libxcb-res0-dev
)
...
PACKAGES_UTILITIES=(
    ...
    flameshot qimgv firefox-esr nala micro xdg-user-dirs-gtk
)
...
if [ "$ONLY_CONFIG" = false ]; then
    ...
    cd dk
    make || die "Failed to build dk"
    sudo make install || die "Failed to install dk"
    msg "dk window manager installed successfully"
else
...
if [ -d "$CONFIG_DIR" ]; then
    clear
    read -p "Found existing dk config. Backup? (y/n) " -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
...
# Copy dk config files
cp "$SCRIPT_DIR"/dk/dkrc "$CONFIG_DIR"/ || die "Failed to copy dkrc"

# Copy configuration directories
for dir in dunst picom polybar rofi scripts sxhkd wallpaper; do
    if [ -d "$SCRIPT_DIR/dk/$dir" ]; then
        cp -r "$SCRIPT_DIR/dk/$dir" "$CONFIG_DIR"/ || die "Failed to copy $dir"
    else
        msg "Warning: dk/$dir directory not found, skipping..."
    fi
done

# Make scripts executable
chmod +x "$CONFIG_DIR"/dkrc
...
echo "1. Log out and select 'dk' from your display manager"
